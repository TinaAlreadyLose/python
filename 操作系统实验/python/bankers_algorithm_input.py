class Bankers_Algorithm:
    ...
    def __init__(self):
        finish=[]
        types=int(input('请输入有几类资源'))
        processes=int(input('请输入有几个进程'))
        allavailable=[]
        for i in range(types):
            a=int(input('请输入第'+str(i)+'资源值'))
            allavailable.append(a)
        ...
        for i in range(processes):
            ...
            for j in range(types):
                a=int(input('请输入第'+str(i)+'个进程的第'+str(j)+'个数据'))
                allocation[i].append(a)
        ...
        for i in range(processes):
            for j in range(types):
                ...
                maxs[i][j] = allocation[i][j] + need[i][j]
        for i in range(types):
    ...
    def _brank(self):
        self._useProcess=int(input('请输入请求的进程编号（0开始）'))
        # request = [1, 0, 0, 0, 1]
        request = []
